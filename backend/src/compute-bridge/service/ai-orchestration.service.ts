...
import { AIProvider, AIResultEntity, AIResultStatus } from '../entities/ai-result-entity';
import { NormalizedScoringResult, ScoringRequestDto, ScoringResponseDto } from '../dto/ai-scoring.dto';
...
export class AIOrchestrationService {
  ...
  async getUserResults(userId: string, limit: number = 10): Promise<AIResultEntity[]> {
  ...
  private async executeScoringAsync(resultId: string, userData: Record<string, any>): Promise<void> {
    ...
    try {
      ...
      const scoringResult = await this.scoreWithFallback(aiResult.provider, userData, aiResult);
