    ...
    return this.withRetry(async () => {
      const response = await this.client.post('/chat/completions', {
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a financial risk assessment expert. Return JSON with creditScore, riskScore, and analysis.',
          },
          {
            role: 'user',
      ...
      return {
        ...
        creditScore: this.normalizeScore(result.creditScore || result.score, 300, 850),
        riskScore: result.riskScore || (100 - this.normalizeScore(result.creditScore, 300, 850)),
